  ...
  const renderClientModal = () => {
    return (
      <Modal
        ...
        size="lg"
      >
        <form onSubmit={isEditing ? handleUpdateClient : handleCreateClient} className="space-y-4">
          {/* Logo Upload */}
          <div className="flex justify-center mb-6">
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              <div className="w-24 h-24 rounded-full bg-white flex items-center justify-center overflow-hidden border-2 border-slate-200 group-hover:border-iris-red/100 transition-colors">
                {clientForm.logo ? (
                  <img src={clientForm.logo} alt="Logo preview" className="w-full h-full object-contain p-2" />
                ) : (
                  <Upload className="w-8 h-8 text-slate-300 group-hover:text-rose-400" />
                )}
              </div>
          ...
          <div>
            ...
            <input required value={clientForm.name} onChange={e => setClientForm({ ...clientForm, name: e.target.value })} type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Industry</label>
              <input value={clientForm.industry} onChange={e => setClientForm({ ...clientForm, industry: e.target.value })} type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Status</label>
              <select value={clientForm.status} onChange={e => setClientForm({ ...clientForm, status: e.target.value as any })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required value={clientForm.email} onChange={e => setClientForm({ ...clientForm, email: e.target.value })} type="email" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Phone</label>
              <input value={clientForm.phone} onChange={e => setClientForm({ ...clientForm, phone: e.target.value })} type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Website</label>
            <input value={clientForm.website} onChange={e => setClientForm({ ...clientForm, website: e.target.value })} type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none" placeholder="https://" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Address</label>
            <input value={clientForm.address} onChange={e => setClientForm({ ...clientForm, address: e.target.value })} type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Account Manager *</label>
            <select value={clientForm.accountManagerId} onChange={e => setClientForm({ ...clientForm, accountManagerId: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none">
          ...
          <div className="pt-2">
            <button type="submit" className="w-full bg-iris-red text-white py-2.5 rounded-lg font-medium hover:bg-rose-700 transition-colors">
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-iris-red text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-rose-700 transition-colors shadow-sm"
          >
        ...
        <div className="flex flex-col md:flex-row md:items-center space-y-3 md:space-y-0 md:space-x-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-iris-red/100"
            />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-iris-red/100"
          >
          ...
          {filteredClients.map((client) => {
            ...
            return (
              <div
                ...
                className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3 active:scale-[0.98] transition-transform"
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-white flex items-center justify-center overflow-hidden border border-iris-red/20 flex-shrink-0">
                      {client.logo ? (
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium border capitalize ${client.status === 'active' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' :
                    client.status === 'lead' ? 'bg-amber-100 text-amber-700 border-amber-200' :
                ...
                <div className="flex justify-between items-center text-sm text-slate-600 border-t border-slate-100 pt-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs">{am ? am.name : 'Unassigned'}</span>
                  </div>
        ...
        <div className="hidden md:block bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <DataTable<Client>
            ...
            onRowClick={(client) => { setSelectedClient(client); setViewMode('detail'); }}
            ...
            columns={[
                ...
                cell: (client) => {
                  ...
                  return (
                    <div className="flex flex-col">
                      <span className="text-xs text-slate-500">LTV: <span className="font-semibold text-slate-700">${lifetimeValue.toLocaleString()}</span></span>
                      {outstanding > 0 && (
                        <span className="text-xs text-amber-600 font-medium">Due: ${outstanding.toLocaleString()}</span>
                      )}
                    </div>
                  );
                }
              },
              { header: 'Industry', accessorKey: 'industry', className: 'text-slate-600' },
              {
              ...
              },
              {
                ...
                cell: (client) => (
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-iris-red/10 text-rose-700">
              ...
              },
              {
                header: 'Status',
                cell: (client) => (
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${client.status === 'active' ? 'bg-emerald-100 text-emerald-800' :
              ...
              },
              {
                ...
                cell: () => (
                  <button className="text-slate-400 hover:text-iris-red transition-colors">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col space-y-4">
        <button onClick={() => setViewMode('list')} className="text-slate-500 hover:text-iris-red flex items-center space-x-2 w-fit">
          <ArrowLeft className="w-4 h-4" />
          <span>Back to Clients</span>
        </button>

        <div className="flex flex-col md:flex-row justify-between items-start gap-4 bg-white p-4 md:p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-start space-x-4 flex-1">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-lg overflow-hidden bg-white flex items-center justify-center border border-iris-red/20 flex-shrink-0">
              {selectedClient.logo ? (
                <img src={selectedClient.logo} alt={selectedClient.name} className="w-full h-full object-contain p-2" />
              ) : (
                <Building2 className="w-6 h-6 md:w-8 md:h-8 text-iris-red" />
              )}
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="text-xl md:text-2xl font-bold text-slate-900 flex flex-wrap items-center gap-2 md:gap-3">
                {selectedClient.name}
                <span className={`text-xs px-2.5 py-0.5 rounded-full border uppercase tracking-wide ${selectedClient.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
              ...
              <p className="text-sm md:text-base text-slate-500 mt-1 truncate">{selectedClient.industry} • {selectedClient.address}</p>
              <div className="flex flex-wrap items-center gap-2 md:gap-4 mt-3 text-xs md:text-sm text-slate-600">
                {selectedClient.website && (
                  <a href={`https://${selectedClient.website}`} target="_blank" rel="noreferrer" className="flex items-center space-x-1 hover:text-iris-red hover:underline">
          ...
          <div className="flex space-x-2">
            <button
              ...
              className="flex items-center space-x-1 md:space-x-2 px-2 md:px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 text-sm font-medium"
            >
      ...
      <div className="border-b border-slate-200">
        <nav className="-mb-px flex space-x-8 overflow-x-auto">
          ...
          <button
            ...
            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'meetings'
              ? 'border-iris-red/100 text-iris-red'
              : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
              }`}
          >
          ...
          {checkPermission('clients.brand_view') && (
            <button
              ...
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center space-x-2 ${activeTab === 'brand'
                ? 'border-iris-red/100 text-iris-red'
                : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
                }`}
            >
              ...
              {selectedClient && brandAssets.filter(a => a.clientId === selectedClient.id).length > 0 && (
                <span className={`px-2 py-0.5 rounded-full text-xs ${activeTab === 'brand'
                  ? 'bg-iris-red/20 text-rose-700'
          ...
          <button
            ...
            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'strategies'
              ? 'border-iris-red/100 text-iris-red'
              : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
              }`}
          >
              ...
              <div className="p-3 md:p-6">
                {currentMonthMilestones.length > 0 ? (
                  <>
                    ...
                    <div className="mt-4 flex flex-wrap justify-center gap-3 md:gap-6 text-xs md:text-sm text-slate-600">
                      <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex bg-slate-100 rounded-lg p-1">
                    <button
                      ...
                      className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${projectTab === 'active' ? 'bg-white text-iris-red shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                      Active
                    </button>
                    <button
                      onClick={() => setProjectTab('archived')}
                      className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${projectTab === 'archived' ? 'bg-white text-iris-red shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                      Archived
                    </button>
                  </div>
                      ...
                      {filteredProjects.map(project => (
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-4">
                            <span className={`text-xs px-2 py-1 rounded ${project.status === 'Active' || project.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
                              }`}>
                              {project.status}
                            </span>
              ...
              <div className="p-5 border-b border-slate-100 flex justify-between items-center">
                ...
                {checkPermission && (checkPermission('clients.manage') || checkPermission('clients.manage_links')) && (
                  <button
                    ...
                    className="flex items-center space-x-2 text-iris-red hover:bg-iris-red/10 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                  >
                  ...
                  {clientSocialLinks.map(link => (
                    <div key={link.id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50 group">
                      ...
                      <div className="col-span-5 truncate">
                        <a href={link.url} target="_blank" rel="noreferrer" className="text-iris-red hover:underline text-sm truncate block">
                          {link.url}
                        </a>
                      </div>
                        ...
                        {checkPermission && (checkPermission('clients.manage') || checkPermission('clients.manage_links')) && (
                          <>
                            <button
                              ...
                              className="p-1.5 text-slate-400 hover:text-iris-red hover:bg-iris-red/10 rounded transition-colors"
                            >
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="bg-white p-4 md:p-5 rounded-xl border border-slate-200 shadow-sm">
            ...
            {am ? (
              <div className="flex items-center space-x-3">
                <img src={am.avatar} alt={am.name} className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover" />
                <div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setIsContactModalOpen(true)} className="text-iris-red hover:bg-iris-red/10 p-1 rounded transition-colors"><Plus className="w-4 h-4" /></button>
            </div>
            <div className="divide-y divide-slate-100">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-medium text-slate-900 text-sm flex items-center gap-2">
                        {contact.name}
                        {contact.isPrimary && <span className="text-[10px] bg-iris-red/20 text-rose-700 px-1.5 py-0.5 rounded-full">Primary</span>}
                      </p>
                ...
                {checkPermission('clients.notes_create') && (
                  <form onSubmit={handleAddNoteHandler} className="mb-4">
                    <textarea
                      ...
                      className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none text-sm min-h-[80px]"
                    />
                    <div className="flex justify-end mt-2">
                      <button
                        type="submit"
                        disabled={!newNoteText.trim()}
                        className="bg-iris-red text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-rose-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Add Note
                      </button>
                    </div>
                        ...
                        {editingNoteId === note.id ? (
                          <div className="space-y-2">
                            <textarea
                              ...
                              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-iris-red/100 focus:outline-none text-sm min-h-[60px]"
                            />
                            <div className="flex justify-end gap-2">
                              ...
                              <button
                                ...
                                className="text-xs bg-iris-red text-white px-2 py-1 rounded hover:bg-rose-700"
                              >
                                Save
                              </button>
                            </div>
                              ...
                              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                {(checkPermission('clients.notes_manage') || (checkPermission('clients.notes_create') && note.createdBy === currentUser?.id)) && (
                                  <button
                                    ...
                                    className="text-slate-400 hover:text-iris-red"
                                    title="Edit Note"
                                  >
