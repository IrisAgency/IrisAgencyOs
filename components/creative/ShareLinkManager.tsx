...
const ShareLinkManager: React.FC<ShareLinkManagerProps> = ({
  ...
  const loadShares = useCallback(async () => {
    ...
    try {
      ...
      // Query by creativeCalendarId if available, otherwise by calendarMonthId
      const q = creativeCalendarId
        ? query(sharesRef, where('creativeCalendarId', '==', creativeCalendarId))
        : calendarMonthId
          ? query(sharesRef, where('calendarMonthId', '==', calendarMonthId))
          : null;
      if (!q) {
        setShares([]);
        setLoading(false);
  ...
  }, [creativeCalendarId, calendarMonthId]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const newShare: Omit<PresentationShare, 'id'> = {
        ...
        creativeCalendarId: creativeCalendarId || null,
        calendarMonthId,
        clientId,
        isActive: true,
        allowDownload: false,
...
const ShareCard: React.FC<ShareCardProps> = ({ share, isCopied, onCopy, onToggle, onRegenerate, isInactive }) => {
  ...
  return (
    <div
      className={`p-3.5 rounded-xl border transition-colors ${isInactive ? 'bg-gray-50 border-gray-100 opacity-60' : 'bg-white border-gray-200 shadow-sm'}`}
    >
      {/* URL display */}
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-gray-400 mb-3">
        ...
        {share.lastOpenedAt && <span>Last opened: {formatDateTime(share.lastOpenedAt)}</span>}
