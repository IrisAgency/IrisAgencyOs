...
import { Plus, Trash2, Search, Copy, Shield, AlertTriangle, CheckCircle, Lock, Save } from 'lucide-react';
import { prefixedId } from '../utils/id';

interface RolesManagerProps {
  roles: RoleDefinition[];
  onAddRole: (role: RoleDefinition) => void;
  onUpdateRole: (role: RoleDefinition) => void | Promise<void>;
...
const RolesManager: React.FC<RolesManagerProps> = ({ roles, onAddRole, onUpdateRole, onDeleteRole, users = [] }) => {
  ...
  // Sync selected role with latest from props — only when not dirty
  React.useEffect(() => {
  ...
  if (!selectedRole && roles.length === 0)
    return (
      <div className="p-8 text-center text-slate-400">
        ...
        <button onClick={() => setIsCreateOpen(true)} className="bg-indigo-600 text-white px-4 py-2 rounded-lg">
          Create First Role
        </button>
      </div>
    );
          ...
          {roles.map((role) => {
            ...
            return (
              <div
                ...
                className={`p-4 cursor-pointer hover:bg-slate-50 flex justify-between items-center group ${selectedRole?.id === role.id ? 'bg-indigo-50 border-l-4 border-indigo-500' : ''}`}
              >
            ...
            <div>
                ...
                {selectedRole.riskLevel && (
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${
                      selectedRole.riskLevel === 'critical'
              ...
              {selectedRole.updatedAt && (
                <p className="text-[10px] text-slate-400 mt-1">
                  Last updated: {new Date(selectedRole.updatedAt).toLocaleDateString()}
                  {selectedRole.updatedBy ? ` by ${selectedRole.updatedBy}` : ''}
                </p>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              {isDirty && (
                <button
                  ...
                  disabled={isSaving}
                  className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-1.5 transition-colors"
                >
