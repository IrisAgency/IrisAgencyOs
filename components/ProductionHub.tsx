...
import { 
  ...
  Search, Film, MoreHorizontal, Settings
} from 'lucide-react';
...
const ProductionHub: React.FC<ProductionHubProps> = ({ 
  assets, shotLists, callSheets, locations, equipment, projects, users,
  onAddShotList, onAddCallSheet, onAddLocation, onAddEquipment, onUpdateEquipment
}) => {
              ...
              {shotLists.map(sl => {
                  ...
                  return (
                      <div key={sl.id} className="border-b border-slate-100 last:border-0">
                          ...
                          {/* Expanded detail for first item in mock */}
                          <div className="bg-white">
                              <table className="w-full text-sm text-left">
                                  <thead className="text-xs text-slate-500 bg-white border-b border-slate-100 uppercase">
                                      <tr>
                                          <th className="px-4 py-2 w-10">#</th>
                                          <th className="px-4 py-2">Shot Desc</th>
                                          <th className="px-4 py-2">Movement</th>
                                          <th className="px-4 py-2">Framing</th>
                                          <th className="px-4 py-2">Lens/Gear</th>
                                      </tr>
                                  </thead>
                                  <tbody className="divide-y divide-slate-50">
                                      {sl.shots.map(shot => (
                                          <tr key={shot.id} className="hover:bg-indigo-50/30">
                                              <td className="px-4 py-3 font-mono text-slate-400">{shot.shotNumber}</td>
                                              <td className="px-4 py-3 font-medium text-slate-800">{shot.description}</td>
                                              <td className="px-4 py-3 text-slate-600">{shot.cameraMovement}</td>
                                              <td className="px-4 py-3 text-slate-600">{shot.framing}</td>
                                              <td className="px-4 py-3 text-slate-500 text-xs">{shot.equipment.length} items</td>
                                          </tr>
                                      ))}
                                  </tbody>
                              </table>
      ...
      <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {callSheets.map(cs => {
                  ...
                  const location = locations.find(l => l.id === cs.locationId);
                  return (
                      <div key={cs.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 hover:border-indigo-300 transition-colors group">
                      ...
                      {equipment.map(eq => (
                          <tr key={eq.id} className="hover:bg-slate-50">
                              ...
                              <td className="px-6 py-4">
                                  <span className="bg-slate-100 text-slate-600 text-xs px-2 py-1 rounded-full">{eq.category}</span>
                              </td>
                              <td className="px-6 py-4 text-slate-500 font-mono text-xs">{eq.serialNumber}</td>
                              <td className="px-6 py-4">
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage shoots, crew, gear, and logistics.</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h2 className="text-lg font-bold text-slate-900">Add {modalType}</h2>
